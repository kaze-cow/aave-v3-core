...
library LiquidationLogic {
    ...
    /**
     * @notice Function to enable the liquidation of a position if its Health Factor drops below 1.
     * In some cases, this function must be called before an account can be liquidated.
     * @dev Emits the `LiquidationCall()` event
     * @param reservesData The state of all the reserves
     * @param reservesList The addresses of all the active reserves
     * @param usersConfig The users configuration mapping that track the supplied/borrowed assets
     * @param eModeCategories The configuration of all the efficiency mode categories
     * @param params The parameters that would be used in call to executeLiquidationCall
     */
    function executeFlagForLiquidation(
        ...
        userConfig.setFlaggedForLiquidation(true);
    }
    ...
    function executeLiquidationCall(
        ...
        require(userConfig.isFlaggedForLiquidation() || userConfig.liquidationFlagTime < block.timestamp - 60, Errors.NOT_FLAGGED_FOR_LIQUIDATION);
        ...
        IERC20(params.debtAsset)
            .safeTransferFrom(msg.sender, vars.debtReserveCache.aTokenAddress, vars.actualDebtToLiquidate);

        IAToken(vars.debtReserveCache.aTokenAddress)
            .handleRepayment(msg.sender, params.user, vars.actualDebtToLiquidate);

        // User no longer is flagged for liquidation now that the liquidation is complete
        userConfig.setFlaggedForLiquidation(false);
    ...
    function _burnCollateralATokens(
        ...
        vars.collateralAToken
            .burn(params.user, msg.sender, vars.actualCollateralToLiquidate, collateralReserveCache.nextLiquidityIndex);
    }
    ...
    function _getConfigurationData(
        ...
        if (params.userEModeCategory != 0) {
            ...
            if (EModeLogic.isInEModeCategory(
